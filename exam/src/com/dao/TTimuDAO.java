...
public class TTimuDAO extends HibernateDaoSupport
{
	...
	private int sqlPageTotle=1;
	public int getSqlPageTotle() {
		return sqlPageTotle;
	}
	public List findBySqlPage(String sql, int pageSize, int pageNow)
	{
		log.debug("finding bysql: " + sql);
		try
		{
			List list=getHibernateTemplate().find(sql);
			int listSize=list.size();
			sqlPageTotle=listSize%pageSize==0?listSize/pageSize:listSize/pageSize+1  ;
			
			int startNum=pageNow*pageSize-pageSize;
			int endNum=startNum+pageSize;		
			if (pageNow==sqlPageTotle )
			{
				endNum=listSize;
			}
			List list1=list.subList(startNum, endNum);
			//System.err.println(startNum+":"+endNum+":"+listSize);
			
			/*
			 Query query =getSessionFactory().getCurrentSession().createQuery(sql);
			
			sqlPageTotle=query.list().size();
			sqlPageTotle=sqlPageTotle%pageSize==0?sqlPageTotle/pageSize:sqlPageTotle/pageSize+1  ;
			
			List list=query.setFirstResult(pageNow*pageSize-pageSize) 
